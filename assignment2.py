...
def main():
    ...
    def downloadData(url):
        """A Function that fetchs data.
        Args:
            url_file: url link where data is found
        """
        url_file = urllib2.urlopen(url)
        return url_file

    def processData(data):
        """Downloads data and creats a new dict.
        Args:
            csv_file: data
            my_dict: new dict of data
            date_format: format that data is displayed
        Return:
            New dict with data (id, name, bday)
        """
    ...
    def displayPerson(id, personData):
        """A function that displays the document."""
